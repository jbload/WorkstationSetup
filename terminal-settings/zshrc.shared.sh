...
gitprune () {
    ...
    if [ ${#branch_names[@]} -gt 0 ]; then
        ...
        if read -q "choice?Remove these branches? (y/n): "; then
...
if command -v pyenv > /dev/null 2>&1; then
    ...
    if command -v brew > /dev/null 2>&1 && MYSQL_CLIENT_PREFIX="$(brew --prefix mysql-client 2> /dev/null)"; then
        export PKG_CONFIG_PATH="$MYSQL_CLIENT_PREFIX/lib/pkgconfig"
    fi
fi
